...
data_path = '/data/lisatmp4/sotelo/data/blizzard/'
file_name = "raw_blizzard_80h.hdf5"

std_file = data_path + 'blizzard_80h' + '_standardize.npz'
len_file = data_path + 'blizzard_80h' + '_length.npy'
...
file_name = "tbptt_blizzard_80h_intermediate.hdf5"
hdf5_path = os.path.join(data_path, file_name)
...
features = intermediateh5file.create_dataset(
           'features', (batch_size, row_size),
           dtype=numpy.dtype('int16'))

phonemes = intermediateh5file.create_dataset(
           'phonemes', (batch_size, row_size),
           dtype=numpy.dtype('int16'))

past_observations = 0
current_file = 0
leftovers = numpy.array([])
left_phon = numpy.array([])

for row in xrange(batch_size):
    data_row = numpy.array([])
    data_row = numpy.hstack([data_row, leftovers])

    phon_row = numpy.array([])
    phon_row = numpy.hstack([phon_row, left_phon])


    while len(data_row) < row_size:
        data_row = numpy.hstack([data_row, h5file['features'][current_file]])
        phon_row = numpy.hstack([phon_row, h5file['phonemes'][current_file]])
    ...
    left_phon = phon_row[row_size:]
    phon_row = phon_row[:row_size]
    phonemes[row] = phon_row
...
features = finalh5file.create_dataset(
           'features',
           (no_minibatches*batch_size, seq_size),
           dtype=numpy.dtype('int16'))

phonemes = finalh5file.create_dataset(
           'phonemes',
           (no_minibatches*batch_size, seq_size),
           dtype=numpy.dtype('int16'))
...
for minibatch in xrange(no_minibatches):
    ...
    phonemes[idx2, :] = intermediateh5file['phonemes'][:, idx1]

features.dims[0].label = 'batch'
features.dims[1].label = 'time'

phonemes.dims[0].label = 'batch'
phonemes.dims[1].label = 'time'
...
split_dict = {
    ...
    'test': {'features': (end_valid, no_minibatches*batch_size),
             'phonemes': (end_valid, no_minibatches*batch_size)}
    }
