...
def _transform_to_sp(data):
    ...
    for x in data[1]:
        x = x.astype('float64')
        mgc_sp = numpy.apply_along_axis(SPTK.mgc2sp, 1, x, alpha, gamma, frame_window).real
        results.append(mgc_sp)

    return tuple([data[0],numpy.array(results).astype('float32')])

data_dir = os.environ['FUEL_DATA_PATH']
data_dir = os.path.join(data_dir, 'blizzard/', 'mgc_standardize.npz')
...
x = tensor.concatenate([mgc,f0s], 2)
