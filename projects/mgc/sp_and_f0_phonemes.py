...
experiment_name = "sp_and_f0_phonemes_2"
...
dataset = Blizzard(which_sets = ('train',), filename = "sp_blizzard_80h_phon.hdf5")
...
dataset = Blizzard(which_sets = ('valid',), filename = "sp_blizzard_80h_phon.hdf5")
data_stream = DataStream.default_stream(
            dataset, iteration_scheme=SequentialScheme(
...
data_stream = Mapping(data_stream, _zero_for_unvoiced)
data_stream = Mapping(data_stream, _transpose)
data_stream = SegmentSequence(data_stream, 4*seq_size, add_flag=True)
data_stream = ForceFloatX(data_stream)
...
for key, value in model.get_parameter_dict().items():
    if key in [name.replace('with_fake_attention','att_trans') 
               for name in main_loop.model.get_parameter_values().keys()]:
      value.set_value(main_loop.model.get_parameter_values()[
              key.replace('att_trans','with_fake_attention')])
    else:
